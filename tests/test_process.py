...
from orc_core.infra.process.process import acquire_lock, kill_orphan_project_processes, release_lock
...
class ProcessLockTest(unittest.TestCase):
    ...
    @patch("orc_core.infra.process.process.is_pid_alive", return_value=False)
...
class OrphanSweepTest(unittest.TestCase):
    @patch("orc_core.infra.process.process.log_event")
    @patch("orc_core.infra.process.process.psutil.wait_procs", return_value=([], []))
    @patch("orc_core.infra.process.process.os.getpid", return_value=999_999)
    def test_orphan_sweep_does_not_match_neighbor_prefix_path(self, _pid_mock, _wait_mock, _log_mock) -> None:
        ...
        with patch("orc_core.infra.process.process.psutil.process_iter", return_value=[neighbor]):
    ...
    def test_orphan_sweep_matches_workspace_subdirectory(self, _pid_mock, _wait_mock, _log_mock) -> None:
        ...
        with patch("orc_core.infra.process.process.psutil.process_iter", return_value=[owned]):
            killed = kill_orphan_project_processes(
                workspace,
                Path("/tmp/orc.log"),
                label="test",
    ...
    def test_orphan_sweep_matches_by_orc_run_token(self, _pid_mock, _wait_mock, _log_mock) -> None:
        ...
        with patch("orc_core.infra.process.process.psutil.process_iter", return_value=[token_proc]):
            killed = kill_orphan_project_processes(
                "/repo/orc",
                Path("/tmp/orc.log"),
                label="test",
    ...
    def test_orphan_sweep_does_not_treat_relative_cmdline_as_workspace_path(self, _pid_mock, _wait_mock, _log_mock) -> None:
        ...
        with patch("orc_core.infra.process.process.psutil.process_iter", return_value=[foreign]):
            killed = kill_orphan_project_processes(
                workspace,
                Path("/tmp/orc.log"),
                label="test",
