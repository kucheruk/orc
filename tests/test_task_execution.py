...
class TaskExecutionEngineTest(unittest.TestCase):
    ...
    @patch("orc_core.tasks.task_execution_launch.wait_for_completion")
    ...
    def test_execute_emits_timeline_start_finish_pairs(
        ...
        original_timeline_step = orc_core.tasks.task_execution.timeline_step
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("orc_core.tasks.task_execution.timeline_step", side_effect=_tracking_timeline_step), \
                 patch("orc_core.tasks.task_execution_launch.timeline_step", side_effect=_tracking_timeline_step):
    ...
    @patch("orc_core.tasks.task_execution_finalize.is_quit_after_task_requested", return_value=True)
    @patch("orc_core.tasks.task_execution_finalize.run_commit_phase", return_value=True)
    @patch("orc_core.tasks.task_agent_phases.kill_process_tree")
    @patch("orc_core.tasks.task_execution.update_task_restart_count")
    @patch("orc_core.tasks.task_execution_resume.write_task_file")
    ...
    @patch("orc_core.tasks.task_agent_phases._git_status_porcelain")
    ...
    @patch("orc_core.tasks.task_agent_phases.kill_process_tree")
    @patch("orc_core.tasks.task_agent_phases.wait_for_process_exit", return_value="completed")
    @patch("orc_core.tasks.task_agent_phases._attempt_autocommit_fallback", return_value=True)
    @patch("orc_core.tasks.task_agent_phases._git_status_porcelain")
    ...
    @patch("orc_core.tasks.task_agent_phases.kill_process_tree")
    @patch("orc_core.tasks.task_agent_phases.wait_for_process_exit", return_value="completed")
    @patch("orc_core.tasks.task_agent_phases._attempt_autocommit_fallback", return_value=False)
    @patch("orc_core.tasks.task_agent_phases._git_status_porcelain")
    ...
    @patch("orc_core.tasks.task_agent_phases._git_status_porcelain", return_value=(True, " M tracked.py\n"))
