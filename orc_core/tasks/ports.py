...
__all__ = [
    ...
    "GitDiffProbe",
    "PreflightResult",
    "MainIntegrationPreflight",
    "IntegrationOutcome",
    "GitIntegrationPort",
]


# ── Stream monitor protocol ────────────────────────────────────────

class StreamMonitorProtocol(Protocol):
    """Full contract for stream monitors — merges metrics, lifecycle, and
    tool-call tracking. Concrete monitors implement this structurally.

    Consumers should depend on the whole protocol; the narrow sub-protocols
    that once existed (MonitorMetrics, MonitorLifecycle, ToolCallManager,
    ProcessProxy) are inlined here since none of them had independent
    consumers.
    """
    ...
    # process lifecycle
    proc: Any  # structural: has .pid, .returncode, .poll()
