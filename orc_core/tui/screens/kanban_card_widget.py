...
class KanbanCardWidget(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(cid, id=f"kc_head_{self.card_id}")
        yield Label("", id=f"kc_role_{self.card_id}")
        yield Label("", id=f"kc_metrics_{self.card_id}")
    ...
    def _refresh_labels(self) -> None:
        ...
        if title:
            ...
            head = safe_id
        self._set(f"kc_head_{c.id}", head)
        ...
        self._set(f"kc_role_{c.id}", role_text)
        ...
        self._set(f"kc_metrics_{c.id}", " ".join(parts) if parts else "")
