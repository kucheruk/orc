...
def _execute_one(
    ...
    if t == "move_card":
        _do_move_card(board, p, action.reason, publisher)
    elif t == "set_action":
        _do_set_action(board, p, action.reason, publisher)
    elif t == "modify_deps":
        _do_modify_deps(board, p, action.reason, publisher)
    elif t == "create_card":
        _do_create_card(board, p, action.reason, publisher)
    elif t == "set_wip_limit":
        _do_set_wip_limit(board, p, action.reason, publisher)
    elif t == "update_card":
        _do_update_card(board, p, action.reason, publisher)
    elif t == "notify":
        _do_notify(board, p, action.reason, publisher, log_path)
    elif t == "skip_card":
        _do_skip_card(board, p, action.reason, publisher)
    elif t == "write_feedback":
        _do_write_feedback(board, p, action.reason, publisher)
    elif t == "respond":
...
def _do_move_card(board, p, reason, publisher):
...
def _do_set_action(board, p, reason, publisher):
    card_id = _require(p, "card_id")
...
def _do_modify_deps(board, p, reason, publisher):
    card_id = _require(p, "card_id")
    card = board.card_by_id(card_id)
    if card is None:
        raise ValueError(f"Card not found: {card_id}")
...
def _do_set_wip_limit(board, p, reason, publisher):
...
def _do_update_card(board, p, reason, publisher):
    card_id = _require(p, "card_id")
...
def _do_skip_card(board, p, reason, publisher):
    card_id = _require(p, "card_id")
    card = board.card_by_id(card_id)
    if card is None:
        raise ValueError(f"Card not found: {card_id}")
